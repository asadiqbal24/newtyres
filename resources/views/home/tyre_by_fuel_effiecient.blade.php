...
<div class="getHelpForm">
                ...
                <div class="search-link">
                    <h1 style="font-size:24px;color:#ee8000;font-weight:bold;padding:0 0 15px 20px;">Energy Efficient Tyres</h1>
                    @foreach($runbrands as $run)
                        @if($run->fuel_tyres->count() > 0)
                        <h2>
                            <a href="{{route('tyres/suv',['slug'=>$run->url])}}">{{$run->name}}</a>
                        </h2>

                        <ul class="search-tyre-size-links">
                            @foreach($run->fuel_tyres as $r)
                                <li style="padding-right:20px">
                                      <a href="{{route('fuel-efficient/view/tyre',['slug'=>$r->url])}}">{{$r->name}}</a>  
                                </li>
                            @endforeach 
                        </ul>
                        @endif
