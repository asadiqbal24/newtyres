                            ...
                            <h1 class="sc-bwzfXH sc-hMqMXs giyoWh">Tyre Brands</h1></div>   
                                    <div>   
                                            ...
                                            @foreach($tyres_brands as $br)      
                                                ...
                                                <h2 class="sc-htpNat sc-kEYyzF xkIpZ">{{$br->name}}</h2>    
                                                <ul class="sc-kgoBCf sc-jwJjzT ktEmEm">
                                                    @foreach($tyres_cat as $cat)       
                                                            
                                                        @php $url = str_replace('/','-',$cat->name); @endphp
                                                                <li class="sc-kpOJdX sc-jRnjsG kDvVJr"><a href="{{route('tyre/size/view',['slug'=>$url])}}" class="sc-VigVT iGmfbA">{{$cat->name}}</a></li> 
